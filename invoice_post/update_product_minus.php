<?php
    ...
    if($_POST)
    {
        ...
        $ProductQuantityMinussql = "UPDATE invoice_product SET product_quantity=product_quantity-1 WHERE invoice_product_id='$invoice_product_id'";
        ...
        if ($conn->query($ProductQuantityMinussql) === TRUE) 
        {
            ...
            //   echo "Record updated successfully";
                ...
                $checkQty = $conn->query("SELECT product_quantity FROM invoice_product WHERE invoice_product_id='$invoice_product_id'");
                if($qRS = $checkQty->fetch_array()){
                    $cqty = (int)$qRS[0];

                    if($cqty == 0){

                        $conn->query("DELETE FROM invoice_product WHERE invoice_product_id='$invoice_product_id'");
