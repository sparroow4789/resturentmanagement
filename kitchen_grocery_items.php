                                        ...
                                        <table id="datatable1" class="display" style="width:100%;">
                                            <thead>
                                                <tr>   
                                                    <th>ID</th>
                                                    <th>Item Name</th>
                                                    <th><font style="float: right;">Available Kitchen Stock</font></th>
                                                    <th><font style="float: right;">Move Date Time</font></th>
                                                </tr>
                                            </thead>
                                            <tbody id="grocery-area">
                                                <?php
                                                    $sql = "SELECT * FROM grocery_kitchen_item gki INNER JOIN grocery_item gi ON gki.grocery_item_id=gi.grocery_item_id ORDER BY gki.grocery_kitchen_item_id DESC";
                                                    $rs=$conn->query($sql);
                                                    while($row =$rs->fetch_array())
                                                    {
                                                        $GroceryKitchenItemId=$row[0];
                                                        $GroceryKitchenItemName=$row[5];
                                                        $GroceryKitchenItemUnit=$row[6];
                                                        $GroceryKitchenItemQuantity=$row[2];
                                                        $GroceryKitchenItemDateTime=$row[3];

                                                ?>
                                                <tr>
                                                    <td><?php echo $GroceryKitchenItemId; ?></td>
                                                    <td><?php echo $GroceryKitchenItemName; ?></td>
                                                    <td><b style="float: right;"><?php echo $GroceryKitchenItemQuantity.' '.$GroceryKitchenItemUnit; ?></b></td>
                                                    <td><font style="float: right;"><?php echo $GroceryKitchenItemDateTime; ?></font></td>
                                                </tr>
                                                <?php } ?>
                                            </tbody>
